	...
	public String patch(Long id, String key, String value, String type, String toType, Boolean remove,
		...
		if (remove) {
			...
			while(i.hasNext()){
				...
				if(p.getName().equals(key) && !p.getToName().equals(value)){
					p.setToName(value);
					p.setVarName(Util.handleDups(varNames, p.getFromVar(), p.getName(), Util.FROM));
					String toParentVar = p.getToParentVar();
					if(toParentVar.equals("")){
						toParentVar = mapping.getToClass().substring(mapping.getToClass().lastIndexOf('.')+1)+"VO";
					}
					p.setToVarName(Util.handleDups(varNames, toParentVar, Util.getParentId(p.getToName()), Util.TO));
					p.setType(type);
					p.setToType(toType);
					found = true;
					break;
			...
			if(!found){
				...
				if(toParentVar.length()==0){
					toParentVar = Util.getVarName(mapping.getToClass().substring(mapping.getToClass().lastIndexOf('.')+1)+"VO");
				}
				toParentVar = Util.handleDups(varNames, toParentVar, Util.getParentId(p.getToName()), Util.TO);
